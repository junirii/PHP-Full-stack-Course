<?php
  ...
  if(isset($_GET['search_w']))
  {
    $sql = "SELECT i_board, title, create_at, views, board_num, writer, img FROM t_board2 WHERE $cate like '%$search_w%' ORDER BY";
  }
  else
  {
    $sql = "SELECT i_board, title, create_at, views, img, board_num, writer FROM t_board2 ORDER BY";
  }
  $sql2 = "LIMIT $start, $list_num";

  $sql_title = "$sql title $sql2";
  $sql_title2 = "$sql title DESC $sql2";
  $sql_time = "$sql create_at $sql2";
  $sql_time2 = "$sql create_at DESC $sql2";
  $sql_view = "$sql views $sql2";
  $sql_view2 = "$sql views DESC $sql2";
  ...
  if(isset($_GET['search_w']))
  {
    $result_paging = mysqli_query($conn, "SELECT i_board FROM t_board2 WHERE $cate like '%$search_w%'");
  }
  else
  {
    $result_paging = mysqli_query($conn, "SELECT i_board FROM t_board2");
  }
  mysqli_close($conn);
...
<body>
  ...
  <?php
    ...
    $board_name = isset($_GET['search_w']) ? "$cate_k(으)로 $search_w 검색 결과" : "게시판2";
  ?>
  <h2><?=$board_name?></h2>
  <a href="write.php?board_num=2"><button>글쓰기</button></a>
    ...
    while($row = mysqli_fetch_assoc($result))
      {
        ...
        if($img)
        {
          print "<td><a href='view_proc.php?i_board=$i_board&board_num=$board_num&hot=0&listing=$listing&list_num=$list_num${search_link}'><img src='img.png' style='width: 15px; height: 15px;'>$title</a></td>";
        }
        else
        {
          print "<td><a href='view_proc.php?i_board=$i_board&board_num=$board_num&hot=0&listing=$listing&list_num=$list_num${search_link}'>$title</a></td>";
        }
        print "<td>$writer</td>";
        print "<td>$date</td>";
        print "<td>$views</td>";
  ...
  //페이저 만들기
    ...
    {$page2 = $block_page_num * ($now_block - 2) + 1;}
    ...
    print "<a href='list2.php?page=$page1&listing=$listing&list_num=$list_num$search_page'><button><<<</button></a>";
    print "<a href='list2.php?page=$page2&listing=$listing&list_num=$list_num$search_page'><button><<</button></a>";
    print "<a href='list2.php?page=$page3&listing=$listing&list_num=$list_num$search_page'><button><</button></a>";

    for($i = $s_pageNum; $i <= $e_pageNum; $i++)
    {
      if($i == $page)
      {
        print "<a href='list2.php?page=$i&listing=$listing&list_num=$list_num$search_page'><b>$i </b></a>";
      }
      else
      {
        print "<a href='list2.php?page=$i&listing=$listing&list_num=$list_num$search_page'>$i </a>";
      }
    ...
    if($now_block < $total_block)
    {$page6 =  $block_page_num * ($total_block - 1) + 1;}
    else
    {$page6 = $total_page;}

    print "<a href='list2.php?page=$page4&listing=$listing&list_num=$list_num$search_page'><button>></button></a>";
    print "<a href='list2.php?page=$page5&listing=$listing&list_num=$list_num$search_page'><button>>></button></a>";
    print "<a href='list2.php?page=$page6&listing=$listing&list_num=$list_num$search_page'><button>>>></button></a>";
    //
  ?>
</body>
</html>