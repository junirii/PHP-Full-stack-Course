<?php
  ...
  if(isset($_GET['search_w']))
  {
    $sql = "SELECT i_board, title, create_at, views, board_num, writer, img FROM t_board WHERE $cate like '%$search_w%' ORDER BY";
  }
  else
  {
    $sql = "SELECT i_board, title, create_at, views, img, board_num, writer FROM t_board ORDER BY";
  }
  ...
  if(isset($_GET['search_w']))
  {
    $result_paging = mysqli_query($conn, "SELECT i_board FROM t_board WHERE $cate like '%$search_w%'");
  }
  else
  {
    $result_paging = mysqli_query($conn, "SELECT i_board FROM t_board");
  }
  //
  //댓글
  $sql_re =
...
<body>
  ...
  <?php
    ...
    $board_name = isset($_GET['search_w']) ? "$cate_k(으)로 $search_w 검색 결과" : "게시판1";
  ?>
  <h2><?=$board_name?></h2>
  ...
  <?php
    if(isset($_GET['search_w']))
    {
      $hidden = "<input type='hidden' name='cate' value='$cate'>
  ...
  <?php  
    ...
    print "<a href='list.php?page=$page1&listing=$listing&list_num=$list_num$search_page'><button><<<</button></a>";
    print "<a href='list.php?page=$page2&listing=$listing&list_num=$list_num$search_page'><button><<</button></a>";
    print "<a href='list.php?page=$page3&listing=$listing&list_num=$list_num$search_page'><button><</button></a>";

    for($i = $s_pageNum; $i <= $e_pageNum; $i++)
    {
      if($i == $page)
      {
        print "<a href='list.php?page=$i&listing=$listing&list_num=$list_num$search_page'><b>$i </b></a>";
      }
      else
      {
        print "<a href='list.php?page=$i&listing=$listing&list_num=$list_num$search_page'>$i </a>";
      }
    ...
    print "<a href='list.php?page=$page4&listing=$listing&list_num=$list_num$search_page'><button>></button></a>";
    print "<a href='list.php?page=$page5&listing=$listing&list_num=$list_num$search_page'><button>>></button></a>";
    print "<a href='list.php?page=$page6&listing=$listing&list_num=$list_num$search_page'><button>>>></button></a>";
    //
  ?>
</body>
</html>